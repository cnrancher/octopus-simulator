...
# The root of the octopus-simulator directory
ROOT_DIR="${CURR_DIR}"
...
function generate() {
  octopus::log::info "generating simulator..."
...
function mod() {
  ...
  octopus::log::info "downloading dependencies for simulator..."
...
function lint() {
  [[ "${1:-}" != "only" ]] && mod
  octopus::log::info "linting simulator..."
...
function build() {
  ...
  octopus::log::info "building simulator(${GIT_VERSION},${GIT_COMMIT},${GIT_TREE_STATE},${BUILD_DATE})..."
...
function package() {
  [[ "${1:-}" != "only" ]] && build
  octopus::log::info "packaging simulator..."

  local repo=${REPO:-rancher}
  local image_name=${IMAGE_NAME:-octopus-simulator}
...
function deploy() {
  [[ "${1:-}" != "only" ]] && package
  octopus::log::info "deploying simulator..."
  ...
  if [[ "${without_manifest}" == "false" ]]; then
    ...
    cp -f "${CURR_DIR}/deploy/e2e/all_in_one.yaml" "${CURR_DIR}/dist/octopus_simulator_all_in_one.yaml"
    sed "s#app.kubernetes.io/version: master#app.kubernetes.io/version: ${tag}#g" \
      "${CURR_DIR}/dist/octopus_simulator_all_in_one.yaml" >"${tmpfile}" && mv "${tmpfile}" "${CURR_DIR}/dist/octopus_simulator_all_in_one.yaml"
    sed "s#image: cnrancher/octopus-simulator:master#image: ${repo}/${image_name}:${tag}#g" \
      "${CURR_DIR}/dist/octopus_simulator_all_in_one.yaml" >"${tmpfile}" && mv "${tmpfile}" "${CURR_DIR}/dist/octopus_simulator_all_in_one.yaml"
  else
...
function test() {
  [[ "${1:-}" != "only" ]] && build
  octopus::log::info "running unit tests for simulator..."
...
function verify() {
  [[ "${1:-}" != "only" ]] && test
  octopus::log::info "running integration tests for simulator..."
...
function e2e() {
  [[ "${1:-}" != "only" ]] && verify
  octopus::log::info "running E2E tests for simulator..."
...
function entry() {
  ...
  for stage in "${stages[@]}"; do
    octopus::log::info "# make simulator ${stage} ${commands}"
    case ${stage} in
